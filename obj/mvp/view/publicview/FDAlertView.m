...
- (instancetype)initWithTitle:(NSString *)title icon:(UIImage *)icon message:(NSString *)message buttonTitles:(NSArray *) titleList callback:(OnButtonClick)cb
{
    if (self = [super initWithFrame:[UIScreen mainScreen].bounds]) {
        _icon = icon;
        _title = title;
        ...
        for(int i=0; i<titleList.count; i++){
            [_buttonTitleArray addObject:titleList[i]];
        }
...
//        _titleLabel.frame = CGRectMake(_iconImageView.frame.origin.x + _iconImageView.frame.size.width + SPACE_SMALL, 1, titleSize.width, titleSize.height);
        _titleLabel.frame = CGRectMake(_iconImageView.frame.origin.x + _iconImageView.frame.size.width, 1, titleSize.width, titleSize.height);
...
- (void)initAllButtons {
    if (_buttonTitleArray.count > 0) {
        ...
        for (NSString *buttonTitle in _buttonTitleArray) {
            ...
            if (index == _buttonTitleArray.count-1 && index != 0) {
